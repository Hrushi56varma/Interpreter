   ...
   boolean ret = false;
	...
	public Object visitExpBin(ExpBinary expBin, Object arg) throws Exception {
		...
		if(v0 instanceof LuaInt && v1 instanceof LuaInt) {
			...
			case REL_NOTEQ :
				return new LuaBoolean(vi0.v != vi1.v);
			default :
				throw new StaticSemanticException(null, "Integer Operator Exception");
		}
		}
			...
			case REL_GT :
				return new LuaBoolean(vs0.value.charAt(0) > vs1.value.charAt(0));
			case REL_EQEQ :
				return new LuaBoolean(vs0.value.matches(vs1.value));
			case REL_NOTEQ :
				return new LuaBoolean(!vs0.value.matches(vs1.value));	
			default :
				throw new StaticSemanticException(null, "String Operator Exception");
		}
		}
			else if(v0 instanceof LuaBoolean && v1 instanceof LuaBoolean) {
			LuaBoolean vb0 = (LuaBoolean) v0;
			LuaBoolean vb1 = (LuaBoolean) v1;
			if(op == KW_and) {
				return new LuaBoolean(vb0.value && vb1.value);
			}
			else if(op == KW_or) {
				return new LuaBoolean(vb0.value || vb1.value);
			}
			else {
				throw new StaticSemanticException(null, "Boolean Operator Exception");
			}
			
		}
		    else if( v0 instanceof LuaInt && v1 instanceof LuaString) {
		    	LuaString vs0 = new LuaString(Integer.toString(((LuaInt) v0).v));
		    	LuaString vs1 = (LuaString) v1;
		    	if(op == DOTDOT) {
		    		return new LuaString(vs0.value + vs1.value);
		    	}
		    	else {
		    		throw new StaticSemanticException(null, "Int, String Operator Exception");
		    	}
		    }
		    else if( v0 instanceof LuaString && v1 instanceof LuaInt) {
		    	LuaString vs1 = new LuaString(Integer.toString(((LuaInt) v1).v));
		    	LuaString vs0 = (LuaString) v0;
		    	if(op == DOTDOT) {
		    		return new LuaString(vs0.value + vs1.value);
		    	}
		    	else {
		    		throw new StaticSemanticException(null, "String, Int Operator Exception");
		    	}
		    }
 
		else {
			throw new StaticSemanticException(null, "Binary Operator Exception");
	}
	}
	...
	public Object visitBlock(Block b, Object arg) throws Exception {
		...
		while(i<sl.size()) {
			...
			Stat s = sl.get(i);
			if( s instanceof RetStat &&!ok && !fg) {
				ret = true;
				retc++;
				return visitRetStat((RetStat)s, arg );
			}
			else if( s instanceof StatAssign &&!ok && !fg) {
			...
			else if (s instanceof StatIf &&!ok && !fg) {
				lbreak.add(2);
				StatIf co = (StatIf) s;
				int nexp = co.es.size();
			...
			else if( s instanceof StatDo &&!ok && !fg) {
				lbreak.add(2);
				StatDo d = (StatDo) s;
				iflist.addAll((List<LuaValue>)visitBlock(d.b, arg));
				lbreak.remove(lbreak.size()-1);
			}
			else if(s instanceof StatWhile &&!ok && !fg) {
				lbreak.add(1);
			...
			else if(s instanceof StatRepeat &&!ok && !fg) {
				lbreak.add(1);
		    ...
		    else if(s instanceof StatGoto &&!ok && !fg) {
				StatGoto g = (StatGoto) s;
				gotoname = g.name.name;
				fg = true;
				ok = true;
			}
			else if(s instanceof StatLabel && fg) {
